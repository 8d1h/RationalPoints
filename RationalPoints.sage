...
def global_height(x, prec=53):
    ...
    if is_RationalField(k):
        finite = lcm([xi.denominator() for xi in x]) / gcd([xi.numerator() for xi in x])
        infinite = max(abs(xi) for xi in x)
        return finite*infinite
    else:
        def ideal_lcm(lst):
            I = lst[0].ring().ideal(1)
            for J in lst:
                I = I.intersection(J)
            return I
        finite = ideal_lcm([xi.denominator_ideal() for xi in x]).norm() / sum([xi.numerator_ideal() for xi in x]).norm()
        d = k.degree()
...
class MyAlgebraicScheme_subscheme(AlgebraicScheme_subscheme):
    def rational_points(self, F=None, bound=None, split=False, tolerance=0.01, prec=53):
        def base_change(k, F):
            ch = F.characteristic()
            if ch == 0:
        ...
        get_elements = lambda : []
        ch = k.characteristic()
        if I.dimension() > 1 or not self.is_projective() and I.dimension() > 0: # positive dimension
        ...
        if self.is_projective(): # projective case
            for pt in enum_proj_points(I):
                if split:
...
def P2_over_number_field(bound=RR(5^(1/3))):
    u = QQ['u'].0
